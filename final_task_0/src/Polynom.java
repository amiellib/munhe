...
public class Polynom implements Polynom_able
{
		...
		{	
			...
			else
			{
				...
				if (i+1<string.length())
				{
					...
					if (string.charAt(i) == '^' && !Character.isDigit(string.charAt(i+1)) || ((string.charAt(i) == 'x' || string.charAt(i) == 'X') && string.charAt(i+1) != '^'))
					{
						throw new wrongDataException("A power can not be a negitave power or a random symbal at location " + (i+1) );
					}
	...
	public double root(double x0, double x1, double eps) 
	{
		...
		for (double current_xt_value = Math.min(x0, x1);current_xt_value<=Math.max(x0, x1);current_xt_value+=eps)
		{
