...
public class Polynom implements Polynom_able
{
	...
	/**
	 * This function calculates and funds the x's value where this function has extremums
	 * @param eps is the smallest gap from the actual result
	 * @param start where the polynom range starts
	 * @param end  where the polynom range ends
	 * @return a ArrayList(Double) of x's value where the extremum exsits
	 */
	public ArrayList<Double> extremum(double start , double end  , double eps) 
	{
		return extremum_polynom(start,end,this,eps);	
	}
	private ArrayList<Double> extremum_polynom(double start , double end , Polynom poly , double eps) 
	{
	 ...
	 */
	private double adjust_round(double value , double eps)
	{
	 ...
	 */
	public void draw(double start,double end , double eps)
	{
		int steps = (int) ((end-start)/eps)+1; // calculate amount of steps 
		ArrayList<Double> extremum_xs = extremum_polynom(start , end ,this ,eps); // get the extremum x's dots
		...
		for (int i=0; i<steps;i++)
		{
			...
			yData[i] = f(start +i*eps);	// y value for graph
		}
		
		XYChart chart = QuickChart.getChart("Sample Chart", "X", "Y", "y(x)", xData, yData); // build initial chart
		for (int i=0; i<extremum_xs.size();i++) // looping through the extremums to add those dots
		{
			chart.addSeries("extremum [" + adjust_round(extremum_xs.get(i),eps)+" , " + adjust_round(f(extremum_xs.get(i)),eps) + " ]"  // define dot location name limited to 3 numbers after the dot
							, new double[] {adjust_round(extremum_xs.get(i),eps)}  // x value of extremum
			, new double[] {adjust_round(f(extremum_xs.get(i)),eps)}, null); // y value of extremum
		}
